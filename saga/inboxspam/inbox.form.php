...
//SourceCode by AndezNET.com
{
    ...
    $data = mysqli_fetch_array(mysqli_query($mysqli,"SELECT * FROM inbox_spam WHERE ID=".$id_inbox));


    if($id_inbox > 0) {
        $id_inbox = $data['ID'];
        $to= $data['SenderNumber'];
		$pesandari= $data['TextDecoded'];
    }
    ...
    <form class="form-horizontal style-form" action="" method="POST" id="form-inbox">
		...
		<div class="form-group">
            ...
            <div class="col-sm-10">
                <textarea class="form-control" name="pesandri" id="pesandri" disabled><?php echo $pesandari ?></textarea>
            </div>
        </div>
	
	
        <div class="form-group">
            <label class="col-sm-2 col-sm-2 control-label">Balas Pesan</label>
            <div class="col-sm-10">
