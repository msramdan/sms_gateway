...
//============================
    ...
    if ($tb_act == "Tambah") {
        if ($name != "" && $datanohp != "") {
            $queryceknohp=mysqli_query($mysqli,"SELECT * FROM pbk where Number='$datanohp'");
            $rowjumcek=mysqli_num_rows($queryceknohp);
            if($rowjumcek==0){
                if(($_FILES['file']['size'] >= $maxsize))
                {
                    ...
                    $q_tambah = mysqli_query($mysqli, "INSERT INTO pbk VALUES ('','$gp','$name','$datanohp','$alamatfile','$tgl_lahir')");
                    if ($q_tambah > 0) {
                ...
                echo "<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'><i class='icon-remove'></i></button><strong><i class='ace-icon fa fa-times'></i> MAAF! </strong> data dengan no hp $datanohp sudah tersedia <br/></div>";
            }
        ...
        if ($name != "" && $datanohp != "") {
            ...
            if(($_FILES['file']['size'] >= $maxsize))
            {
                ...
                if (($_FILES['file']['size'] == 0) ){
                    ...
                    $q_edit1 = mysqli_query($mysqli, "UPDATE pbk SET GroupID='$gp', Name='$name', Number='$datanohp',tgl_lahir='$tgl_lahir' where ID='$idpbk'");
                    if ($q_edit1 > 0) {
                        echo "<div class='alert alert-block alert-success'><button type='button' class='close' data-dismiss='alert'><i class=''></i></button><strong><i class='ace-icon fa fa-check'></i> BERHASIL</strong> Phonebook Berhasil disimpan<br/></div>";
                    } else {
                        echo "<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'><i class='icon-remove'></i></button><strong><i class='ace-icon fa fa-times'></i> MAAF! </strong>" . mysqli_error($mysqli) . "<br/></div>";
                    ...
                    $q_edit2 = mysqli_query($mysqli, "UPDATE pbk SET GroupID='$gp',Name='$name',Number='$datanohp',Foto='$alamatfile',tgl_lahir='$tgl_lahir' where ID='$idpbk'");
                    if ($q_edit2 > 0) {
                        echo "<div class='alert alert-block alert-success'><button type='button' class='close' data-dismiss='alert'><i class=''></i></button><strong><i class='ace-icon fa fa-check'></i> BERHASIL</strong> Phonebook Berhasil disimpan<br/></div>";
                    } else {
                        echo "<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'><i class='icon-remove'></i></button><strong><i class='ace-icon fa fa-times'></i> MAAF! </strong>" . mysqli_error($mysqli) . "<br/></div>";
...
//SourceCode by AndezNET.com
    ...
    <div>
        ...
        <a href="?id=pb" class="btn btn-info">
            <i class="fa fa-refresh"></i>
            Refresh
        </a>
		
		<a  href="?id=import"class="btn btn-success" >
            <i class="fa fa-file-excel-o"></i>
            Import Xls
        </a>
    ...
    <div id="dialog-balaspesan" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
